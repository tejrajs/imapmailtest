...
namespace app\modules\mail\components\imap;
...
class Mailbox extends component{
	...
	protected function connect() {
		$imapStream = @imap_open($this->imapPath.$this->folder, $this->imapLogin, $this->imapPassword, $this->imapOptions, $this->imapRetriesNum, $this->imapParams);
		if(!$imapStream) {
	...
	public function checkMailbox() {
		return imap_check($this->connect());
	}
	...
	public function createMailbox() {
		return imap_createmailbox($this->connect(), imap_utf7_encode($this->imapPath));
	}
	...
	public function statusMailbox() {
		return imap_status($this->connect(), $this->imapPath, SA_ALL);
	}
	...
	public function getListingFolders() {
		$folders = imap_list($this->connect(), $this->imapPath, "*");
	...
	public function searchMailbox($criteria = 'ALL') {
		$mailsIds = imap_search($this->connect(), $criteria, SE_UID, $this->serverEncoding);
	...
	public function saveMail($mailId, $filename = 'email.eml') {
		return imap_savebody($this->connect(), $filename, $mailId, "", FT_UID);
	}
	...
	public function moveMail($mailId, $mailBox) {
		return imap_mail_move($this->connect(), $mailId, $mailBox, CP_UID) && $this->expungeDeletedMails();
	}
	...
	public function expungeDeletedMails() {
		return imap_expunge($this->connect());
	}
	...
	public function setFlag(array $mailsIds, $flag) {
		return imap_setflag_full($this->connect(), implode(',', $mailsIds), $flag, ST_UID);
	}
	...
	public function clearFlag(array $mailsIds, $flag) {
		return imap_clearflag_full($this->connect(), implode(',', $mailsIds), $flag, ST_UID);
	}
	...
	public function getMailsInfo(array $mailsIds) {
		$mails = imap_fetch_overview($this->connect(), implode(',', $mailsIds), FT_UID);
	...
	public function getMailboxInfo() {
		return imap_mailboxmsginfo($this->connect());
	}
	...
	public function sortMails($criteria = SORTARRIVAL, $reverse = true) {
		return imap_sort($this->connect(), $criteria, $reverse, SE_UID);
	}
	...
	protected function getQuota() {
		return imap_get_quotaroot($this->connect(), 'INBOX');
	}
	...
	protected function initMailPart(IncomingMail $mail, $partStructure, $partNum, $markAsSeen = true) {
		...
		$data = $partNum ? imap_fetchbody($this->connect(), $mail->id, $partNum, $options) : imap_body($this->connect(), $mail->id, $options);
		...
		if($attachmentId) {
			...
			if($this->attachmentsDir) {
				$replace = array(
						'/\s/' => '_',
						'/[^0-9a-zÃƒï¿½Ã‚Â°-Ãƒâ€˜Ã¯Â¿Â½Ãƒâ€˜Ã¢â‚¬â€œÃƒâ€˜Ã¢â‚¬â€�Ãƒâ€˜Ã¢â‚¬ï¿½_\.]/iu' => '',
						'/_+/' => '_',
						'/(^_)|(_$)/' => '',
				);
